...
const Provider: React.FC = ({ children }) => {
  ...
  const  bnbcBnbLpV1PoolAddress = tsuki ? tsuki.contracts.bnbcBnbLpV1Pool.options.address : ''
  const bnbcBnbLpV1Address = tsuki ? tsuki.contracts.bnbcBnbLpV1.options.address : ''

  const { isApproved, isApproving, onApprove } = useApproval(
    bnbcBnbLpV1Address,
    bnbcBnbLpV1PoolAddress,
    () => setConfirmTxModalIsOpen(false)
  )

  const fetchEarnedBalance = useCallback(async () => {
    if (!account || !tsuki) return
    const balance = await getEarned(tsuki, tsuki.contracts.bnbcBnbLpV1Pool, account)
  ...
  const fetchStakedBalance = useCallback(async () => {
    ...
    const balance = await getStaked(tsuki, tsuki.contracts.bnbcBnbLpV1Pool, account)
  ...
  const handleHarvest = useCallback(async () => {
    if (!tsuki) return
    setConfirmTxModalIsOpen(true)
    await harvest(tsuki, 'bnbcBnbLpV1', account, () => {
      setConfirmTxModalIsOpen(false)
  ...
  const handleRedeem = useCallback(async () => {
    if (!tsuki) return
    setConfirmTxModalIsOpen(true)
    await redeem(tsuki, 'bnbcBnbLpV1', account, () => {
      setConfirmTxModalIsOpen(false)
      setIsRedeeming(true)
    })
  ...
  const handleStake = useCallback(async (amount: string) => {
    if (!tsuki) return
    setConfirmTxModalIsOpen(true)
    await stake(tsuki, 'bnbcBnbLpV1', amount, account, () => {
      setConfirmTxModalIsOpen(false)
      setIsStaking(true)
    })
    setIsStaking(false)
  ...
  const handleUnstake = useCallback(async (amount: string) => {
    if (!tsuki) return
    setConfirmTxModalIsOpen(true)
    await unstake(tsuki, 'bnbcBnbLpV1', amount, account, () => {
      setConfirmTxModalIsOpen(false)
      setIsUnstaking(true)
    })
